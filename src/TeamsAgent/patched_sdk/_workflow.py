...
class Workflow(DictConvertible):
    ...
    async def _send_responses_internal(self, responses: dict[str, Any]) -> None:
        ...
        # Validate and convert responses against pending requests
        ...
        for request_id, response in responses.items():
            ...
            # PATCH: If response is a dict, try to construct the expected type from it
            if isinstance(response, dict) and not isinstance(response, pending_request.response_type):
                try:
                    import dataclasses
            ...
            converted_responses[request_id] = response

        await asyncio.gather(*[
            self._runner_context.send_request_info_response(request_id, response)
            for request_id, response in converted_responses.items()
        ])
