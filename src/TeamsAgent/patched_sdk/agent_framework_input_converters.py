...
from agent_framework._types import TextContent, FunctionResultContent
        ...
        try:
            if isinstance(input, list):
                ...
                for item in input:
                    ...
                    # Case 3: function_call_output (HITL response from Teams/Foundry)
                    elif item.get("type") == "function_call_output":
                        call_id = item.get("call_id")
                        output = item.get("output")
                        if call_id and output is not None:
                            logger.debug("Processing function_call_output: call_id=%s", call_id)
                            ...
                            if isinstance(output, str):
                                try:
                                    ...
                                    pass
                            # Create a FunctionResultContent and wrap in ChatMessage
                            func_result = FunctionResultContent(
                                call_id=call_id,
                                result=result_data
                            )
                            messages.append(ChatMessage(role=ChatRole.TOOL, contents=[func_result]))
